...
public:
        ...
        , m_startZ(0)
    {
        ...
        ROS_INFO("Comm delay is %d", delay);
    }
    ...
    std::vector<tf::StampedTransform> leaderVector;
        ...
        case Automatic:
            {
                ...
                leaderVector.push_back(transformLeader);

                ROS_INFO("1, %f %f ", transformLeader.getOrigin().x(), transformLeader.getOrigin().y());

                /*for(std::vector<tf::StampedTransform>::iterator asdf = leaderVector.begin(); asdf != leaderVector.end(); asdf++)
                {
                     ROS_INFO("%f %f ", asdf->getOrigin().x(), asdf->getOrigin().y());

                }*/
                if(leaderVector.size() < delay)
                {
                    break;
                }
                transformLeader = leaderVector.front();
                leaderVector.erase(leaderVector.begin());
                ...
                ROS_INFO("2, %f %f ", transformLeader.getOrigin().x(), transformLeader.getOrigin().y());
