...
public:
    ...
    Follower(
        ...
        const std::string& frame_follower,
        const ros::NodeHandle& n)
        : m_worldFrame(worldFrame)
        , m_frame(frame_follower)
        , m_frame_leader(frame_leader)
        , m_getTwistData()
        , m_pubNav()
        , m_listener()
        , m_pidX(
        ...
        case GoToZDesired:
            {
                tf::StampedTransform transform;
                m_listener.lookupTransform(m_worldFrame, m_frame, ros::Time(0), transform);
                m_listener_leader.lookupTransform(m_worldFrame, m_frame_leader, ros::Time(0), transform2);
...
int main(int argc, char **argv)
{
  ...
  std::string frame_follower;
  n.getParam("frame", frame_follower);
  std::string frame_leader;
  n.getParam("frame_leader", frame_leader);
  double frequency;
  n.param("frequency", frequency, 50.0);

  Follower follower(worldFrame, frame_follower, n);
